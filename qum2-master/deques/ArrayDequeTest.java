package deques;

import org.junit.Test;

import static org.junit.Assert.assertEquals;

public class ArrayDequeTest {

    @Test
    public void testTricky() {
        ArrayDeque<Integer> deque = new ArrayDeque<>();
        ArrayDeque<Integer> deque2 = new ArrayDeque<>();
        ArrayDeque<Integer> deque3 = new ArrayDeque<>();
        ArrayDeque<Integer> deque4  = new ArrayDeque<>();

        deque2.addFirst(6);
        assertEquals(6, (int) deque2.get(0));
        deque2.addFirst(7);
        assertEquals(7, (int) deque2.get(0));
        deque2.addFirst(8);
        assertEquals(8, (int) deque2.get(0));
        deque2.addFirst(9);
        deque2.addFirst(10);
        deque2.addFirst(11);
        deque2.addFirst(12);
        assertEquals(12, (int) deque2.get(0));
        deque2.addFirst(13);
        assertEquals(13, (int) deque2.get(0));
        deque2.addFirst(14);
        assertEquals(14, (int) deque2.get(0));

        assertEquals(14, (int) deque2.removeFirst());
        assertEquals(13, (int) deque2.removeFirst());
        assertEquals(12, (int) deque2.removeFirst());
        assertEquals(11, (int) deque2.removeFirst());
        assertEquals(10, (int) deque2.removeFirst());
        assertEquals(9, (int) deque2.removeFirst());
        assertEquals(8, (int) deque2.removeFirst());




        deque.addFirst(0);
        assertEquals(0, (int) deque.get(0));

        deque.addLast(1);
        assertEquals(1, (int) deque.get(1));

        deque.addFirst(-1);
        deque.addLast(2);
        assertEquals(2, (int) deque.get(3));

        deque.addLast(3);
        deque.addLast(4);

        // Test that removing and adding back is okay
        assertEquals(-1, (int) deque.removeFirst());
        deque.addFirst(-1);
        assertEquals(-1, (int) deque.get(0));

        deque.addLast(5);
        deque.addFirst(-2);
        deque.addFirst(-3);

        // Test a tricky sequence of removes
        assertEquals(-3, (int) deque.removeFirst());
        assertEquals(5, (int) deque.removeLast());
        assertEquals(4, (int) deque.removeLast());
        assertEquals(3, (int) deque.removeLast());
        assertEquals(2, (int) deque.removeLast());
        // Failing test
        assertEquals(1, (int) deque.removeLast());

        //my test

    }
}
